...
SKILL.md provides overview and quick start. Separate files hold detailed
reference material.

```
skill-name/
  SKILL.md              # Overview + quick start + links to detail
  reference.md          # Detailed API reference
  examples.md           # Usage examples
  advanced.md           # Advanced features
```

In SKILL.md, link to each file with a description:

```markdown
**API reference**: Complete method signatures and parameters for the data
processing API. See [reference.md](reference.md)

**Examples**: Common usage patterns with runnable code.
See [examples.md](examples.md)

**Advanced features**: Caching, batching, and webhook configuration.
...
SKILL.md links each domain file with a description of its contents. When a user
asks about revenue, the agent reads only `reference/finance.md`.

## Pattern 3: Conditional Details

Show basic content inline. Link to advanced content for specific scenarios.
...
**For tracked changes**: Redline markup rules and OOXML change-tracking
format. See [redlining.md](redlining.md)

**For OOXML internals**: Raw XML structure and namespace reference.
...
- **One level deep**: All files link directly from the overview (SKILL.md or
  index.md). Never chain overview -> intermediate -> actual content.
...
SKILL.md -> advanced.md    (contains the detail directly)
SKILL.md -> details.md     (also linked from overview)
```
